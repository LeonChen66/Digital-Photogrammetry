function y=inter_Matrix(i,j)
global f xa ya XL YL ZL XA YA ZA ini x0 y0 B_dot_ij B_dot2_ij m11 m12 m13 m21 m22 m23 m31 m32 m33 q r s b11 b12 b13 b14 b15 b16 J K b21 b22 b23 b24 b25 b26 omega phi kappa  allphoto_1
f = 18.378;
x0 =0;
y0 = 0;
omega = deg2rad(ini(6*i-5));
phi = deg2rad(ini(6*i-4));
kappa = deg2rad(ini(6*i-3));
XL = ini(6*i-2);
YL = ini(6*i-1);
ZL = ini(6*i);
XA = ini(3*j+allphoto_1*6-2);
YA = ini(3*j+allphoto_1*6-1);
ZA = ini(3*j+allphoto_1*6);
delta_X = XA - XL;
delta_Y = YA - YL;
delta_Z = ZA - ZL;
